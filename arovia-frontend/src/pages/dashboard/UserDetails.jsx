...
function UserDetails() {
  ...
  const vitals = [
    { 
      label: "Blood Group", 
      // Formats B_POSITIVE -> B+ and O_NEGATIVE -> O-
      value: user.bloodGroup ? user.bloodGroup.replace("_POSITIVE", "+").replace("_NEGATIVE", "-") : "—", 
      icon: Droplet, 
      color: "red" 
    },
    ...
    <>
      ...
      <div className="ud-profile-card">
        <div className="ud-avatar">
          {user.avatar ? <img src={user.avatar} alt={user.name} /> : (user.name || "U").charAt(0)}
        </div>
        <div className="ud-profile-info">
          <div className="ud-profile-name">{user.name}</div>
          <div className="ud-profile-meta">
            <span><IdCard size={12} /> {user.userId}</span>
            <span>Created {user.createdAt ? new Date(user.createdAt).toLocaleDateString() : "—"}</span>
            <span>Updated {user.updatedAt ? new Date(user.updatedAt).toLocaleDateString() : "—"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="ud-info-container">
        <div className="ud-grid">
            ...
            <div className="ud-field-list">
              {fields.map(({ key, label, icon: Icon }) => (
                <div className="ud-field" key={key}>
                  <div className="ud-field-icon"><Icon size={15} /></div>
                  <div className="ud-field-body">
                    <div className="ud-field-label">{label}</div>
                    <div className="ud-field-value">{user[key] || "—"}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="ud-side">
              ...
              <div className="ud-vitals-grid">
                {vitals.map(({ label, value, icon: Icon, color }) => (
                  <div className="ud-vital" key={label}>
                    <div className={`ud-vital-icon ${color}`}><Icon size={16} /></div>
            ...
            <div className="ud-card">
              <div className="ud-card-title"><ShieldAlert size={15} /> Medical Summary</div>
...
function InfoChips({ icon: Icon, label, values = [], className }) {
  return (
    <div className="ud-med-block">
      <div className="ud-med-label">{Icon && <Icon size={13} />} {label}</div>
      <div className="ud-chip-row">
        {(values || []).length ? (
          values.map(v => <span className={`ud-chip ${className}`} key={v}>{v}</span>)
        ) : (
          <span className="ud-field-value">None recorded</span>
        )}
      </div>
    </div>
  );
